...
for(i = 0; i < 8; i++) {
    ...
    if is_string(global.bossNames[i]) and global.bossNames[i] != "" {
        ...
        if string_length(word) <= 6 {
            ...
            draw_text(xx[i] - _vx, yy[i] + 48 - _vy, string_hash_to_newline(word));
        }
        else {
...
if !drawLogo {
    ...
    if global.stageSelectFollow {
        ...
        if oldOption != option or idx > 0 {
			if idx == 0 or oldOption != option {
				idx = 1;
			}
			else if !instance_exists(objAchievementBox) {
				idx += global.speedStageSelect;
				if (idx >= sprite_get_number(global.spriteStageSelect)) idx = 0;
			}
        }
    }
...
if global.enableScrews && showShop >= 1 {
    ...
    var posy = yy[8] + 52;
    if global.GP == -1 {
        draw_text(posx, posy, string_hash_to_newline(string_upper(key_to_string(global.selectKey)) + "] SHOP"));
    }
    else {
        draw_text(posx, posy, string_hash_to_newline(string_upper(button_to_string(global.selectButton)) + "] SHOP"));
    }
}
