if !global.frozen {
	switch phase {
	    ...
	    case 7: //Show weapon name
	        ...
	        if timer >= 25 {
	            timer = 0;
	            if changeColAtUtil || cfgWeaponPreview || !doAgain {
					phase = 8;
				}
				else {
					phase = 9;
	    ...
	    case 9: //Weapon color and timer for leaving
			...
			{
				if !doAgain
					_demoTime = global.weaponID.demoTime;
				else
					_demoTime = global.utilityID.demoTime;
			}
			else
			{
	...
	&& !doAgain {
	    ...
	    if phase == 8 {   //Flicker
			if timer mod 4 == 0 or timer mod 4 == 2 {
	...
	and doAgain {
	    ...
	    if phase == 8 {   //Flicker
	        ...
	        if global.currentWeapon == global.utilityID.newID {
