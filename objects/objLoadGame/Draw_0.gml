...
if !surePhase {
	for (var i = 1; i < num_saves + 1; i++) {
		...
		else if !errorPhase && !successPhase {
	...
	if !errorPhase && !successPhase {
		...
		var _action = "";
		switch(action_index) {
			case 0: _action = "LOADED"; break;
			case 2: _action = "OVERWRITTEN"; break;
			case 3: _action = "DELETED"; break;
		}
		if action_index != 2 || isOverwrite {
			draw_text(round(room_width / 2), top + 3, string_hash_to_newline("FILE " + string(selected) + " " + _action + "."));
		}
		else {
			draw_text(round(room_width / 2), top + 3, string_hash_to_newline("FILE " + string(copiedIndex) + " COPIED."));
		}
	}
	else {
		draw_set_halign(fa_center);
		draw_set_colour(c_white);
		draw_text(round(room_width / 2), top + 3, string_hash_to_newline("ERROR!!"));
	}
}
else {
	for (var i = 1; i < num_saves + 1; i++) {
