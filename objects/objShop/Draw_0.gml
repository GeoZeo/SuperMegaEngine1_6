...
for (var i = 0; i < total_items_; i++) {
    ...
    draw_text(currX, currY+17, string_hash_to_newline(string_replace_all(string_format(global.items[i].price, 3, 0), " ", "0")));
...
draw_text(56, 105, string_hash_to_newline(string_replace_all(string_format(global.screws, 3, 0), " ", "0")));

//Exit
draw_rectangle(175, 95, 214, 104, c_black);
draw_set_color(c_black);
draw_text(183, 96, string_hash_to_newline("EXIT"));
...
if option < total_items {
    //Item Name
    draw_text(8, 182, string_hash_to_newline(string_upper(global.items[item_index].name) + ": 0" + string(global.items[item_index].count) + " / MAX: 0" + string(global.items[item_index].maxUnits)) );
