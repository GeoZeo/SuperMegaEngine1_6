...
{
    draw_sprite_ext(sprHealthbarBackground, 0, _viewX+7+8+5, _viewY+17, 1, 1, 0, c_black, 1);
    
    for(i = 1; i <= global._health; i += 1)
    {
        draw_sprite_ext(sprHealthbarPrimary, 0, _viewX+7+8+1+5, _viewY+17+(sprite_get_height(sprHealthbarBackground)-i*2), 1, 1, 0, make_color_rgb(252, 228, 160), 1);
        draw_sprite_ext(sprHealthbarSecondary, 0, _viewX+7+8+1+5, _viewY+17+(sprite_get_height(sprHealthbarBackground)-i*2), 1, 1, 0, make_color_rgb(248, 248, 248), 1);
    }
}
else
{
    for (i = 1; i <= global._maxHealth; i += 1)
    {
        if (i < global._maxHealth)
            draw_rectangle_color(_viewX+7+8+5, _viewY+17+((sprite_get_height(sprHealthbarBackground)+2)-i*2), _viewX+7+8+5+7, _viewY+17-3+(sprite_get_height(sprHealthbarBackground)-i*2), c_black, c_black, c_black, c_black, false);
        else
            draw_rectangle_color(_viewX+7+8+5, _viewY+17+((sprite_get_height(sprHealthbarBackground)+2)-i*2), _viewX+7+8+5+7, _viewY+17-1+(sprite_get_height(sprHealthbarBackground)-i*2), c_black, c_black, c_black, c_black, false);
...
if global.weapon != 0 {
	if (global.maxAmmo == 28)
	{
	    ...
	    draw_sprite_ext(sprHealthbarBackground, 0, _viewX+7+5, _viewY+17, 1, 1, 0, global.outlineCol, 1);
    
	    for(i = 1; i <= ceil(global.weapons[global.currentWeapon].ammo); i += 1) {
	        draw_sprite_ext(sprHealthbarPrimary, 0, _viewX+7+1+5, _viewY+17+(sprite_get_height(sprHealthbarBackground)-i*2), 1, 1, 0, global.primaryCol, 1);
	        draw_sprite_ext(sprHealthbarSecondary, 0, _viewX+7+1+5, _viewY+17+(sprite_get_height(sprHealthbarBackground)-i*2), 1, 1, 0, global.secondaryCol, 1);
	    }
	}
	else
	{
	    ...
	    {
	        if (i < global.maxAmmo)
	            draw_rectangle_color(_viewX+7+5, _viewY+17+((sprite_get_height(sprHealthbarBackground)+2)-i*2), _viewX+7+5+7, _viewY+17-3+(sprite_get_height(sprHealthbarBackground)-i*2), global.outlineCol, global.outlineCol, global.outlineCol, global.outlineCol, false);
	        else
	            draw_rectangle_color(_viewX+7+5, _viewY+17+((sprite_get_height(sprHealthbarBackground)+2)-i*2), _viewX+7+5+7, _viewY+17-1+(sprite_get_height(sprHealthbarBackground)-i*2), global.outlineCol, global.outlineCol, global.outlineCol, global.outlineCol, false);
...
if !(myStartingPoint > -1 or myTimer > -1) {
	if instance_exists(prtPlayer) && !prtPlayer.teleporting && !prtPlayer.showReady && !prtPlayer.landing && !prtPlayer.dead {
...
if cfgShowLives {
    ...
    draw_text(_viewX+7+8+5, _viewY+18+sprite_get_height(sprHealthbarBackground), string_hash_to_newline(string(max(0,global._lives))));
...
if (cfgDebug || debug_mode) {
	...
	draw_set_halign(fa_left);
    draw_set_valign(fa_top);
    draw_text( _viewX+7+8+8+5+16, _viewY+17+16, "X: " + string(x));
	draw_text( _viewX+7+8+8+5+16, _viewY+34+16, "Y: " + string(y));
	draw_set_font(_oldFont);
	draw_set_halign(_oldHalign);
	draw_set_valign(_oldValign);
}
