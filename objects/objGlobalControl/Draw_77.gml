...
if window_get_fullscreen() && global.screen_border != noone
{
	...
	var _border = asset_get_index("sb" + _borderStr + "_" + string(global.displayWidth) + "x" + string(global.displayHeight));
	if !sprite_exists(_border)
		_border = asset_get_index("sb" + _borderStr);
		
	if sprite_exists(_border)
	{
		var _borderName = sprite_get_name(_border);
		
		if string_contains(_borderName, "_")
		{
			draw_sprite_ext(_border, 0, 0, 0, 1, 1, 0, c_white, 1);
		}
		else
		{
