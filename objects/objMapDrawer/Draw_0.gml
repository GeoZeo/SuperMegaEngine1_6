var keys_size = array_length_1d(keys);
...
for (var i = -1; i <= step; i++) {
	...
	if i == step {
		...
		if draw_speed > 0 && step > -1 && canDraw {
			if playDrawSound {
	...
	if i > -1 {
		for (var j = 0; j < lines_size; j++) {
			var current_lines = ds_list_find_value(lines, j);
			if is_array(current_lines) {
				if i >= current_lines[4] {
				...
				for (var k = 0; k < size; k++) {
					var line = ds_list_find_value(current_lines, k);
					if i == line[4] {
						var dx = line[0] + p * (line[2] - line[0]);
			            var dy = line[1] + p * (line[3] - line[1]);
			            draw_line_width(x + line[0], y + line[1], x + dx, y + dy, line_width+2);
		...
		for (var j = 0; j < lines_size; j++) {
			...
			if is_array(current_lines) {
				if i >= current_lines[4] {
			        ...
			        var dy = line[1] + p * (line[3] - line[1]);
					_p = p;
			        draw_line_width(x + (line[0] - sign(line[2] - line[0])), y + (line[1] - sign(line[3] - line[1])), x + dx, y + dy, line_width);
				}
			}
			else {
				var size = ds_list_size(current_lines);
...
if first draw_sprite(sprFortressPoint, 0, x, y);
var last_line = ds_list_find_value(lines, lines_size - 1);
if !hidden || (draw_speed > 0 && drawPoint) {
...
if draw_speed > 0 && point_frame + 0.1 < 2 && canDraw {
    point_frame += 0.1;
}
else {
    point_frame = 0;
