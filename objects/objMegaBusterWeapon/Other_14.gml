...
if (!instance_exists(objSectionSwitcher) or cfgChargeWhileLocked) && (chargeTimer != 0 or initChargeTimer != 0) {
	...
	if prtPlayer.canShoot {
		if chargeTimer != 0 {
			...
			&& (instance_number(objBusterShotCharged) + _chargeShotsDeflected) < 1 && (instance_number(objBusterShot) + instance_number(objBusterShotHalfCharged) + _halfChargeShotsDeflected + _noChargeShotsDeflected) < maxshots {
