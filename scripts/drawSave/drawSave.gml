...
function drawSave(argument0, argument1, argument2) {
	...
	if map > -1 and selected {
	    ...
	    //Draw Screws
	    if cfgEnableScrews {
	        draw_text(room_width - margin - 26, y1 + 16, string_hash_to_newline(string_lpad(string(map[? "screws"]),3,"0")));
	    ...
	    for (var i = 0; i < array_length_1d(global.items); i++) {
	        ...
	        if count > 0 || i == 0 {
	            ...
	            if global.items[i].showCount {
	                draw_text(x1 + 4 + i * 8, y1 + 38, string_hash_to_newline(string(count)));
	            }
	        }
	    }    
